...
_Example:_ What is the collection of all polynomials that have $[1]+[2]$ as the divisor of roots? The answer is the set \\[\\{k(x-1)(x-2)\| k \in \mathbb{R}\\}\\] and it can be proved using factor theorem. By setting $k=1$, we see that a monic polynomial $x^2-3x+2$ is uniquely determined by the divisor $[1]+[2]$.
